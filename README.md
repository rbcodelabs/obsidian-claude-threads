...
![Claude Threads](https://img.shields.io/badge/Obsidian-Plugin-7C3AED) ![Version](https://img.shields.io/badge/version-0.32.2-blue) [![Roadmap](https://img.shields.io/badge/Roadmap-Compass-6366F1)](https://compass.rbcodelabs.com/portal/rbcodelabs/claude-threads/roadmap)
...
Open the **Agents List** from the ribbon or command palette to see all threads at a glance. Its fixed header shows the filtered thread count and compact Search and Group controls; search opens in a separate row below the header so the title bar stays stable. The Group menu has independent **Project** and **Status** toggles, supporting Project-only, Status-only, or the default Project + Status hierarchy. At least one grouping remains enabled, and the choice is saved for the next session. In Status-only mode, each row shows its resolved Project as context.
...
> **Scheduled work dashboard:** **Settings → Scheduled** provides a durable “Next up” view sorted by each item's persisted next-run time, with both a relative countdown and exact local timestamp. Gated work is labeled **Next check** because a successful evaluation is not guaranteed to spawn a thread; overdue work is labeled **Overdue — catching up**. The dashboard separates recurring jobs from thread loops and one-shot wakeups, shows active hours, project, working directory, gate details, and recent outcomes, and provides pause/resume, delete, and **Open last run** controls. Use **Create with Claude** to open a normal thread that helps define and create a schedule conversationally.
